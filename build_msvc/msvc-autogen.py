...
def parse_config_into_orn_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Orion Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/orion_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/orion_config.h'), "w", encoding="utf8") as orn_config:
        orn_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Orion-core msbuild configuration initialiser.')
    ...
    parse_config_into_orn_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/orion_config.h'), os.path.join(SOURCE_DIR, 'config/orion-config.h'))
