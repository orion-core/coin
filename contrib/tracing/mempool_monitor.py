...
""" Example logging Orion Core mempool events using the mempool:added,
...
def main(oriond_path):
    oriond_with_usdts = USDT(path=str(oriond_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    oriond_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    oriond_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    oriond_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    oriond_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[oriond_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/oriond")
        exit(1)
