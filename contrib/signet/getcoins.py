...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing orion-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='orion-cli', help='orion-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Orion address to which the faucet should send')
...
parser.add_argument('orion_cli_args', nargs='*', help='Arguments to pass on to orion-cli (default: -signet)')

args = parser.parse_args()

if args.orion_cli_args == []:
    args.orion_cli_args = ['-signet']


def orion_cli(rpc_command_and_params):
    argv = [args.cmd] + args.orion_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = orion_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = orion_cli(['getnewaddress', 'faucet', 'bech32'])
