...
TMPDIR_PREFIX = "orion_func_test_"
...
class OrionTestMetaClass(type):
    """Metaclass for OrionTestFramework.

    Ensures that any attempt to register a subclass of `OrionTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'OrionTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("OrionTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("OrionTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class OrionTestFramework(metaclass=OrionTestMetaClass):
    """Base class for a orion test script.

    Individual orion test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave orionds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop orionds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use orion-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_oriond = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "oriond" + config["environment"]["EXEEXT"],
        )
        fname_orioncli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "orion-cli" + config["environment"]["EXEEXT"],
        )
        fname_orionutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "orion-util" + config["environment"]["EXEEXT"],
        )
        self.options.oriond = os.getenv("ORIOND", default=fname_oriond)
        self.options.orioncli = os.getenv("ORIONCLI", default=fname_orioncli)
        self.options.orionutil = os.getenv("ORIONUTIL", default=fname_orionutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: orionds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("OrionRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'oriond', self.options.oriond) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'orion-cli', self.options.orioncli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a oriond test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple oriond test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as oriond's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    oriond=self.options.oriond,
                    orion_cli=self.options.orioncli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in orion.conf
    ...
    def skip_if_no_oriond_tracepoints(self):
        """Skip the running test if oriond has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("oriond has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_oriond_zmq(self):
        """Skip the running test if oriond has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("oriond has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if orion-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("orion-wallet has not been compiled")

    def skip_if_no_orion_util(self):
        """Skip the running test if orion-util has not been compiled."""
        if not self.is_orion_util_compiled():
            raise SkipTest("orion-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if orion-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("orion-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether orion-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether orion-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_orion_util_compiled(self):
        """Checks whether orion-util was compiled."""
        return self.config["components"].getboolean("ENABLE_ORION_UTIL")
