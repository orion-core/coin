This directory contains integration tests that test oriond and its
...
oriond and orion-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (orion-util, orion-tx, ...).
...
Before tests can be run locally, Orion Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the oriond nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another oriond
process running on the system (perhaps from a previous test which hasn't successfully
killed all its oriond nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other oriond processes are running.

On linux, the test framework will warn if there is another
oriond process running when the tests are started.

If there are zombie oriond processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
oriond processes running on the system, so should not be used if any non-test
oriond processes are being run.**
...
sure oriond processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and oriond `debug.log`s will all be dumped
...
`test_framework.log` and oriond `debug.log`s can be combined into a single
...
call methods that interact with the oriond nodes-under-test.

If further introspection of the oriond instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/oriond.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/oriond <pid>
```
