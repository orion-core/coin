...
const char * const ORION_CONF_FILENAME = "orion.conf";
const char * const ORION_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //orion-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", ORION_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Orion
    // macOS: ~/Library/Application Support/Orion
    // Unix-like: ~/.orion
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Orion";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Orion";
#else
    // Unix-like
    return pathRet / ".orion";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", ORION_CONF_FILENAME));
}
