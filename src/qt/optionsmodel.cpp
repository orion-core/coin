...
/** Call node.updateRwSetting() with Orion 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Orion 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/orion/orion/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because orion 22.x will not try to read these.
...
//! Convert enabled/size values to orion -prune setting.
...
//! Get pruning enabled value to show in GUI from orion -prune setting.
...
//! Get pruning size value to show in GUI from orion -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayOrionUnit")) {
        settings.setValue("DisplayOrionUnit", QVariant::fromValue(OrionUnit::ORN));
    }
    QVariant unit = settings.value("DisplayOrionUnit");
    if (unit.canConvert<OrionUnit>()) {
        m_display_orion_unit = unit.value<OrionUnit>();
    } else {
        m_display_orion_unit = OrionUnit::ORN;
        settings.setValue("DisplayOrionUnit", QVariant::fromValue(m_display_orion_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if orion.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_orion_unit);
...
{
    if (new_unit.isNull() || new_unit.value<OrionUnit>() == m_display_orion_unit) return;
    m_display_orion_unit = new_unit.value<OrionUnit>();
    QSettings settings;
    settings.setValue("DisplayOrionUnit", QVariant::fromValue(m_display_orion_unit));
    Q_EMIT displayUnitChanged(m_display_orion_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/orion/orion/pull/8273
    ...
    // (https://github.com/orion-core/gui/issues/567).
