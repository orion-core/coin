...
const std::string OrionGUI::DEFAULT_UIPLATFORM =
...
OrionGUI::OrionGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &OrionGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &OrionGUI::showModalOverlay);
...
void OrionGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Orion address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and orion: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &OrionGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &OrionGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &OrionGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Orion addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Orion addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Orion Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Orion Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a orion: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Orion command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &OrionGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &OrionGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &OrionGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &OrionGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &OrionGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &OrionGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &OrionGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &OrionGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &OrionGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &OrionGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &OrionGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &OrionGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &OrionGUI::showDebugWindow);
}

void OrionGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void OrionGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &OrionGUI::setCurrentWalletBySelectorIndex);
...
void OrionGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &OrionGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &OrionGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &OrionGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &OrionGUI::showProgress);
...
#ifdef ENABLE_WALLET
void OrionGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void OrionGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &OrionGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &OrionGUI::removeWallet);
...
WalletController* OrionGUI::getWalletController()
{
    return m_wallet_controller;
}

void OrionGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &OrionGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &OrionGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &OrionGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &OrionGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &OrionGUI::incomingTransaction);
    connect(this, &OrionGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void OrionGUI::removeWallet(WalletModel* walletModel)
{
...
void OrionGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void OrionGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void OrionGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void OrionGUI::setWalletActionsEnabled(bool enabled)
{
...
void OrionGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &OrionGUI::toggleHidden);
...
void OrionGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void OrionGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void OrionGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void OrionGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void OrionGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void OrionGUI::openClicked()
{
...
void OrionGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void OrionGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void OrionGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void OrionGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void OrionGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void OrionGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void OrionGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void OrionGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Orion network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void OrionGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void OrionGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void OrionGUI::updateHeadersSyncProgressLabel()
{
...
void OrionGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void OrionGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &OrionGUI::quitRequested);
...
void OrionGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void OrionGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void OrionGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &OrionGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &OrionGUI::show);
                e->ignore();
            }
        }
    }
...
void OrionGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void OrionGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void OrionGUI::incomingTransaction(const QString& date, OrionUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(OrionUnits::formatWithUnit(unit, amount, true));
...
void OrionGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void OrionGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool OrionGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool OrionGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void OrionGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void OrionGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void OrionGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void OrionGUI::updateWindowTitle()
{
...
void OrionGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void OrionGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void OrionGUI::detectShutdown()
{
...
void OrionGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void OrionGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(OrionGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = OrionGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void OrionGUI::subscribeToCoreSignals()
{
...
void OrionGUI::unsubscribeFromCoreSignals()
{
...
bool OrionGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<OrionUnit> units = OrionUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const OrionUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, OrionUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const OrionUnit u : OrionUnits::availableUnits()) {
        menu->addAction(OrionUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(OrionUnit newUnits)
{
    setText(OrionUnits::longName(newUnits));
}
