...
#include <qt/orionaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Orion address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new OrionAddressEntryValidator(parent));
    widget->setCheckValidator(new OrionAddressCheckValidator(parent));
}
...
bool parseOrionURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no orion: URI
    if(!uri.isValid() || uri.scheme() != QString("orion"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!OrionUnits::parse(OrionUnit::ORN, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseOrionURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseOrionURI(uriInstance, out);
}

QString formatOrionURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("orion:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(OrionUnits::format(OrionUnit::ORN, info.amount, false, OrionUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openOrionConf()
{
    ...
    /* Open orion.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Orion.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Orion (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Orion (%s).lnk", chain));
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "orion.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("orion-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a orion.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Orion\n";
        else
            optionFile << strprintf("Name=Orion (%s)\n", chain);
