...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<OrionUnit>("OrionUnit");
#else
    qRegisterMetaType<OrionUnit>("OrionUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. orion_de.qm (shortcut "de" needs to be defined in orion.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. orion_de_DE.qm (shortcut "de_DE" needs to be defined in orion.qrc)
...
static const char* qt_argv = "orion-qt";

OrionApplication::OrionApplication()
...
void OrionApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the OrionApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", OrionGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void OrionApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool OrionApplication::createOptionsModel(bool resetSettings)
{
...
void OrionApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new OrionGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &OrionGUI::quitRequested, this, &OrionApplication::requestShutdown);
...
void OrionApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void OrionApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool OrionApplication::baseInitialize()
{
    return node().baseInitialize();
}

void OrionApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &OrionApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &OrionApplication::handleRunawayException);
    connect(this, &OrionApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &OrionApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void OrionApplication::parameterSetup()
{
...
void OrionApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void OrionApplication::requestInitialize()
{
...
void OrionApplication::requestShutdown()
{
...
void OrionApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // orion: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &OrionGUI::handlePaymentRequest);
            connect(window, &OrionGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void OrionApplication::handleRunawayException(const QString &message)
{
...
void OrionApplication::handleNonFatalException(const QString& message)
{
...
WId OrionApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool OrionApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", OrionGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(orion);
    Q_INIT_RESOURCE(orion_locale);
    ...
    /// 6-7. Parse orion.conf, determine network, switch to network specific
    ...
    // orion: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
