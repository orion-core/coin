...
Below are some notes on how to build Orion Core for Windows.

The options known to work for building Orion Core on Windows are:
...
Note that for WSL the Orion Core source path MUST be somewhere in the default mount file system, for
example /usr/src/orion, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\orion`, for example:

    make install DESTDIR=/mnt/c/workspace/orion
