# CJDNS support in Orion Core

It is possible to run Orion Core over CJDNS, an encrypted IPv6 network that
...
done outside of Orion Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Orion network and individual nodes.
...
before it will work with your Orion Core node. This is described in steps
...
## Run Orion Core with CJDNS

Once you are connected to the CJDNS network, the following Orion Core
...
When enabled, this option tells Orion Core that it is running in an
...
IPv6 local network. This helps Orion Core perform better address management:
...
CJDNS support was added to Orion Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `orion-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/orion/orion/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Orion Core

There are several ways to see your CJDNS address in Orion Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `orion-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `orion-cli getpeerinfo`).
