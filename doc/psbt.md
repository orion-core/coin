# PSBT Howto for Orion Core

Since Orion Core 0.17, an RPC interface exists for Partially Signed Orion
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/orion/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Orion transactions that are not fully signed
...
#### Multisig with multiple Orion Core instances
...
Orion Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `orion-cli` is used.
...
Later, when *V* ORN has been received on *Amulti*, and Bob and Carol want to
