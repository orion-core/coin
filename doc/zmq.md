...
The Orion Core daemon can be configured to act as a trusted "border
router", implementing the orion wire protocol and relay, making
...
The ZeroMQ feature in Orion Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building oriond:
...
For instance:
    ...
    $ oriond -zmqpubhashtx=tcp://127.0.0.1:219627 \
               -zmqpubhashtx=tcp://192.168.1.2:219627 \
               -zmqpubhashblock="tcp://[::1]:219628" \
               -zmqpubrawtx=ipc:///tmp/oriond.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in orion.conf.
...
From the perspective of oriond, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into oriond directly. Furthermore, no information is
...
using. Oriond appends an up-counting sequence number to each
