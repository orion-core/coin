# User-space, Statically Defined Tracing (USDT) for Orion Core

Orion Core includes statically defined tracepoints to allow for more
...
**Note**: Orion Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Orion Core
...
Multiple tools can list the available tracepoints in a `oriond` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Orion Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/oriond
stap net        outbound_message 0x0000000000107c05 /src/oriond
stap validation block_connected  0x00000000002fb10c /src/oriond
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Orion Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/oriond | grep NT_STAPSDT -A 4 -B 2
